...
if ($conn->query($sql_create_id) === TRUE) {
	...
	$sql_insert_portal_parms = "INSERT INTO Checklist.Portal_Parms(portal_id, portal, insights, opsportal, plweb11, plweb12, plweb13, plweb14, eu1_q, eu2_q, remedy, kpi, edtws, mkt_intel, fore_admin, sms_bill, rdm_vp, sys_rpt, eftws,Notable_Failures) VALUES ($last_id, ".IfEmpty($_POST[portal]).", ".IfEmpty($_POST[insights]).", ".IfEmpty($_POST[opsportal]).", ".IfEmpty($_POST[plweb11]).", ".IfEmpty($_POST[plweb12]).", ".IfEmpty($_POST[plweb13]).", ".IfEmpty($_POST[plweb14]).", ".IsPostNull($_POST[eu1_q]).", ".IsPostNull($_POST[eu2_q]).", ".IfEmpty($_POST[remedy]).", ".IfEmpty($_POST[kpiapi]).", ".IfEmpty($_POST[edtws]).", ".IfEmpty($_POST[MktIntel]).", ".IfEmpty($_POST[ForeAdmin]).", ".IfEmpty($_POST[SMSBill]).", ".IfEmpty($_POST[RDMVP]).", ".IfEmpty($_POST[SysRpt]).", ".IfEmpty($_POST[EFTWS]).",\"$_POST[Notable_Failures]\")";
...
//    Submitting to Parm Tables
    ...
    if ($conn->query($sql_insert_data_parms) === TRUE) {} else {
        echo "<br>Data Parms Error: " . $conn->error;
        $AllSubmissionSuccess=FALSE;
    }

    if ($conn->query($sql_insert_dial_parms) === TRUE) {} else {
